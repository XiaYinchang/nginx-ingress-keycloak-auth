...
import (
	...
	authconfig "github.com/XiaYinchang/nginx-ingress-keycloak-auth/config"
...
func NewOauthProxyApp() *cli.App {
	...
	app := cli.NewApp()
	app.Name = common.Prog
	app.Usage = common.Description
	...
	app.Action = func(cx *cli.Context) error {
		...
		if configFile != "" {
			if err := authconfig.ReadConfigFile(configFile, config); err != nil {
				return util.PrintError("unable to read the configuration file: %s, error: %s", configFile, err.Error())
			}
		}

		// step: parse the command line options
		if err := parseCLIOptions(cx, config); err != nil {
			return util.PrintError(err.Error())
		}
...
func getCommandLineOptions() []cli.Flag {
	...
	var flags []cli.Flag
	count := reflect.TypeOf(authconfig.Config{}).NumField()
	for i := 0; i < count; i++ {
		field := reflect.TypeOf(authconfig.Config{}).Field(i)
		...
		if envName != "" {
			envName = common.EnvPrefix + envName
		}
...
func parseCLIOptions(cx *cli.Context, config *authconfig.Config) (err error) {
	...
	for i := 0; i < count; i++ {
		...
		if util.ContainedIn(name, ignoredOptions) {
			continue
		}
	...
	if cx.IsSet("match-claims") {
		...
		util.MergeMaps(config.MatchClaims, claims)
	}
	if cx.IsSet("headers") {
		headers, err := util.DecodeKeyPairs(cx.StringSlice("headers"))
		if err != nil {
			return err
		}
		util.MergeMaps(config.Headers, headers)
	}
	if cx.IsSet("resources") {
		for _, x := range cx.StringSlice("resources") {
			resource, err := resource.NewResource().Parse(x)
			if err != nil {

...
func (r *OauthProxy) proxyMiddleware(next http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		scope := req.Context().Value(common.ContextScopeName)
		if scope != nil {
			sc := scope.(*usercontext.RequestScope)
			if sc.AccessDenied {
				return
			}
		}
		...
		req.Header.Add("X-Forwarded-For", util.RealIP(req))
		...
		if util.IsUpgradedConnection(req) {
			r.Log.Debug("upgrading the connnection", zap.String("client_ip", req.RemoteAddr))
			if err := util.TryUpdateConnection(req, w, r.endpoint); err != nil {
...
func (r *OauthProxy) forwardProxyHandler() func(*http.Request, *http.Response) {
	...
	go func() {
		for {
			...
			if state.wait {
				...
				duration := util.GetWithin(state.expiration, 0.85)
	...
	return func(req *http.Request, resp *http.Response) {
		...
		if len(r.config.ForwardingDomains) == 0 || util.ContainsSubString(hostname, r.config.ForwardingDomains) {
			req.Header.Set("Authorization", fmt.Sprintf("Bearer %s", state.token.Encode()))
			req.Header.Set("X-Forwarded-Agent", common.Prog)
		}
	}
}

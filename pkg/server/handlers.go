...
func (r *OauthProxy) getRedirectionURL(w http.ResponseWriter, req *http.Request) string {
	...
	case "":
		...
		scheme := common.UnsecureScheme
		if req.TLS != nil {
			scheme = common.SecureScheme
		}
		// @QUESTION: should I use the X-Forwarded-<header>?? ..
		redirect = fmt.Sprintf("%s://%s",
			util.DefaultTo(req.Header.Get("X-Forwarded-Proto"), scheme),
			util.DefaultTo(req.Header.Get("X-Forwarded-Host"), req.Host))
	default:
		redirect = r.config.RedirectionURL
	}

	state, _ := req.Cookie(common.RequestStateCookie)
...
func (r *OauthProxy) oauthAuthorizationHandler(w http.ResponseWriter, req *http.Request) {
	...
	if util.ContainedIn("offline", r.config.Scopes) {
		accessType = "offline"
	}
...
func getClientAuthMethod(authMethod string) string {
	switch authMethod {
	case common.AuthMethodBasic:
		return oauth2.AuthMethodClientSecretBasic
	case common.AuthMethodBody:
...
func (r *OauthProxy) oauthCallbackHandler(w http.ResponseWriter, req *http.Request) {
	...
	common.OauthTokensMetric.WithLabelValues("issued").Inc()
	...
	if r.config.EnableRefreshTokens && resp.RefreshToken != "" {
		...
		encrypted, err = util.EncodeText(resp.RefreshToken, r.config.EncryptionKey)
		if err != nil {
	...
	if req.URL.Query().Get("state") != "" {
		if encodedRequestURI, _ := req.Cookie(common.RequestURICookie); encodedRequestURI != nil {
...
func (r *OauthProxy) loginHandler(w http.ResponseWriter, req *http.Request) {
	errorMsg, code, err := func() (string, int, error) {
		...
		common.OauthLatencyMetric.WithLabelValues("login").Observe(time.Since(start).Seconds())
		...
		// @metric a token has been issued
		common.OauthTokensMetric.WithLabelValues("login").Inc()

		w.Header().Set("Content-Type", "application/json")
		if err := json.NewEncoder(w).Encode(common.TokenResponse{
...
func (r *OauthProxy) logoutHandler(w http.ResponseWriter, req *http.Request) {
	...
	common.OauthTokensMetric.WithLabelValues("logout").Inc()
	...
	revocationURL := util.DefaultTo(r.config.RevocationEndpoint, revokeDefault)
	...
	if r.config.EnableLogoutRedirect {
		...
		if redirectURL == "" {
			...
			if r.config.RedirectionURL != "" {
				...
				redirectURL = util.GetRequestHostURL(req)
			}
		}
	...
	if revocationURL != "" {
		...
		common.OauthLatencyMetric.WithLabelValues("revocation").Observe(time.Since(start).Seconds())
...
func (r *OauthProxy) healthHandler(w http.ResponseWriter, req *http.Request) {
	w.Header().Set(common.VersionHeader, common.GetVersion())
	w.WriteHeader(http.StatusOK)
	_, _ = w.Write([]byte("OK\n"))
}
...
func (r *OauthProxy) proxyMetricsHandler(w http.ResponseWriter, req *http.Request) {
	if r.config.LocalhostMetrics {
		if !net.ParseIP(util.RealIP(req)).IsLoopback() {
			r.accessForbidden(w, req)
			return
		}
	}
...
func (r *OauthProxy) retrieveRefreshToken(req *http.Request, user *usercontext.UserContext) (token, encrypted string, err error) {
	switch r.useStore() {
	case true:
	...
	token, err = util.DecodeText(token, r.config.EncryptionKey)
	return
}

...
func TestHasAccessOK(t *testing.T) {
	...
	for i, x := range cs {
		assert.True(t, util.HasAccess(x.Need, x.Have, x.Required), "case: %d should be true, have: %v, need: %v, require: %t ", i, x.Have, x.Need, x.Required)
	}
}
...
func TestHasAccessBad(t *testing.T) {
	...
	for i, x := range cs {
		assert.False(t, util.HasAccess(x.Need, x.Have, x.Required), "case: %d should be false, have: %v, need: %v, require: %t ", i, x.Have, x.Need, x.Required)
	}
}
...
func TestIsUpgradedConnection(t *testing.T) {
	header := http.Header{}
	header.Add(common.HeaderUpgrade, "")
	assert.False(t, IsUpgradedConnection(&http.Request{Header: header}))
	header.Set(common.HeaderUpgrade, "set")
...
func TestIdValidHTTPMethod(t *testing.T) {
	...
	for _, x := range cs {
		assert.Equal(t, x.Ok, util.IsValidHTTPMethod(x.Method))
	}
}
...
`,
	...
	for i, test := range testCases {
		...
		err := config.ReadConfigFile(file.Name(), config)
		if test.Ok && err != nil {
			os.Remove(file.Name())
